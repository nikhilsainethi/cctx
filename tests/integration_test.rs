...
fn pipeline_analyze_then_optimize_all_presets_then_diff() {
    ...
    for preset in &["safe", "balanced", "aggressive"] {
        ...
        let opt_out = cctx()
            .args(["optimize", fixture, "--preset", preset, "--output", out_path.to_str().unwrap()])
            .output().unwrap();
        ...
        let diff_out = cctx()
            .args(["diff", fixture, out_path.to_str().unwrap(), "--format", "json"])
            .output().unwrap();
        assert!(diff_out.status.success(), "diff failed for preset {}", preset);
...
fn piped_optimize_then_count_shows_reduction_for_structural_content() {
    ...
    let orig_count: usize = String::from_utf8_lossy(
        &cctx().args(["count", fixture]).output().unwrap().stdout
    ).trim().parse().unwrap();
    ...
    let new_count: usize = String::from_utf8_lossy(
        &cctx().args(["count"]).write_stdin(opt.stdout).output().unwrap().stdout
    ).trim().parse().unwrap();
...
fn budget_compression_within_5_percent_margin() {
    ...
    for budget in &budgets {
        let opt = cctx()
            ...
            .output().unwrap();
        assert!(opt.status.success(), "compress failed for budget {}", budget);
...
fn system_message_survives_all_strategies() {
    ...
    for strategy in &strategies {
        ...
        let has_system = msgs.iter().any(|m|
            ...
            && m["content"].as_str().map_or(false, |c| c.contains("database expert"))
        );
        assert!(has_system, "system message missing after --strategy {}", strategy);
    }

    // Also test combined aggressive preset.
    let out = cctx()
        .args(["optimize", "--preset", "aggressive", "--prune-threshold", "0.5"])
        .write_stdin(input.clone())
        .output().unwrap();
    assert!(out.status.success());
    let msgs: Vec<Value> = serde_json::from_slice(&out.stdout).unwrap();
    ...
    assert!(has_system, "system message missing after compress --budget 100");
}
...
fn output_roundtrip_write_then_read_back() {
    ...
    let re_opt = cctx()
        .args(["optimize", out_path.to_str().unwrap(), "--preset", "balanced"])
        .output().unwrap();
...
fn balanced_reduces_more_than_bookend_alone_on_structured_content() {
    ...
    let bookend_count: usize = String::from_utf8_lossy(
        &cctx().args(["count"]).write_stdin(bookend_out.stdout).output().unwrap().stdout
    ).trim().parse().unwrap();
    ...
    let balanced_count: usize = String::from_utf8_lossy(
        &cctx().args(["count"]).write_stdin(balanced_out.stdout).output().unwrap().stdout
    ).trim().parse().unwrap();
...
fn structural_optimization_is_idempotent() {
    ...
    let count1: usize = String::from_utf8_lossy(
        &cctx().args(["count"]).write_stdin(opt1.stdout.clone()).output().unwrap().stdout
    ).trim().parse().unwrap();
    ...
    let count2: usize = String::from_utf8_lossy(
        &cctx().args(["count"]).write_stdin(opt2.stdout).output().unwrap().stdout
    ).trim().parse().unwrap();
...
fn bookend_preserves_all_messages() {
    ...
    let mut content1: Vec<String> = msgs1.iter()
        .filter_map(|m| m["content"].as_str().map(|s| s.to_string()))
        .collect();
    let mut content2: Vec<String> = msgs2.iter()
        .filter_map(|m| m["content"].as_str().map(|s| s.to_string()))
        .collect();
    content1.sort();
    content2.sort();
    assert_eq!(content1, content2, "bookend should preserve all message content");
}
...
fn aggressive_on_large_input_completes_without_panic() {
    let fixture = "tests/fixtures/large_conversation.json";
    let out = cctx()
        .args(["optimize", fixture, "--preset", "aggressive", "--prune-threshold", "0.5"])
        .output().unwrap();
    assert!(out.status.success());
    let msgs: Vec<Value> = serde_json::from_slice(&out.stdout).unwrap();
